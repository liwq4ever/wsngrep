...
parse_packet(u_char* info, const struct pcap_pkthdr* header, const u_char* packet)
{
    ...
    if (pkt->proto == IPPROTO_UDP) {
        ...
        // Set packet ports
        pkt->src.port = htons(tcp->source);
        pkt->dst.port = htons(tcp->dest);
...
capture_packet_reasm_ip(capture_info_t* capinfo, const struct pcap_pkthdr* header, u_char* packet, uint32_t* size, uint32_t* caplen)
{
    ...
    if (capinfo->link == DLT_EN10MB) {
        struct _ETHERNET_HEADER* eth = (struct _ETHERNET_HEADER*)packet;
        if (ntohs(eth->Type) == ETHERTYPE_8021Q) {
            link_hl += 4;
        }
    }
    ...
    while (*size >= sizeof(struct ip)) {
        ...
        switch (ip_ver) {
        case 4:
            ip_hl = ip4->HeaderLength * 4;
            ip_proto = ip4->Protocol;
            ip_off = ntohs(ip4->FlagsAndOffset);
            ip_len = ntohs(ip4->TotalLength);

            ip_frag = ip_off & (IP_MF | IP_OFFMASK);
            ip_frag_off = (ip_frag) ? (ip_off & IP_OFFMASK) * 8 : 0;
            ip_id = ntohs(ip4->Identification);

            inet_ntop(AF_INET, &ip4->SourceAddress, src.ip, sizeof(src.ip));
            inet_ntop(AF_INET, &ip4->DestinationAddress, dst.ip, sizeof(dst.ip));
            break;
#ifdef USE_IPV6
        case 6:
    ...
    if (pkt->ip_cap_len == pkt->ip_exp_len) {
        ...
        while ((frame = vector_iterator_next(&it))) {
            ...
            case 4: {
                ...
                len_data += ntohs(frame_ip->TotalLength) - frame_ip->HeaderLength * 4;
                break;
            }
#ifdef USE_IPV6
            case 6: {
        ...
        while ((frame = vector_iterator_next(&it))) {
            ...
            case 4: {
                // Get IP header
                struct ip* frame_ip = (struct ip*)(frame->data + link_hl);
                memcpy(packet + link_hl + ip_hl + (ntohs(frame_ip->FlagsAndOffset) & IP_OFFMASK) * 8,
                    frame->data + link_hl + frame_ip->HeaderLength * 4,
                    ntohs(frame_ip->TotalLength) - frame_ip->HeaderLength * 4);
